...
export async function handleCommand(msg, text, sendMessage) {
  ...
  if (text.toUpperCase() === 'CI') {
    ...
    const lines = runs.map(r => {
      ...
      return `- ${status} — ${r.name} — ${r.html_url}`;
    });
  ...
  // FIX
  {
    ...
    if (m) {
      ...
      await sendMessage(chatId, `🧹 Generating patch plan for *${path}*…`);
      ...
      return sendMessage(
        chatId,
        [
          '*✅ Patch proposal ready (no changes applied yet).*',
          `Repo: *${owner}/${repo}*`,
          `Branch: \`${branch}\``,
          '',
          '*Planned changes:*',
